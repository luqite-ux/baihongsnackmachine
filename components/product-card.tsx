...
export function ProductCard({ product, bareListItem = false }: { product: Product; bareListItem?: boolean }) {
    ...
    <div className="group h-full">
      ...
      >
        ...
        <div className="border-t border-neutral-200 bg-[#f3f3f3] px-4 py-3 text-center transition-colors group-hover:bg-[#f39a00] group-hover:text-white">
          ...
          {product.summary && (
            <p className="mt-1 line-clamp-1 text-xs text-neutral-500 group-hover:text-white/85">{resolveText(product.summary)}</p>
          )}
        </div>
      </Link>
    </div>
